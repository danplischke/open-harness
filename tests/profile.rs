...
sources:
  - caps
";
    let profile = Profile::from_text(text).expect("a YAML profile parses");
...
fn a_profile_is_found_without_being_named() {
    ...
    // A short name alone is found...
    write(&wd.join("oh.yaml"), "name: x\n");
    assert_eq!(profile::find_profile(&wd), Some(wd.join("oh.yaml")));

    // ...but the canonical name wins when both are present, and both are
    // reported so an ambiguous project can be flagged rather than guessed at.
    write(&wd.join("open-harness.yaml"), "name: y\n");
    assert_eq!(
        profile::find_profile(&wd),
        Some(wd.join("open-harness.yaml"))
    );
    assert_eq!(profile::all_profiles(&wd).len(), 2);
    let _ = std::fs::remove_dir_all(&wd);
}
...
fn yaml_emission_round_trips_a_profile() {
    ...
    let text = open_harness::yaml::to_document(&value);
    assert!(
        text.starts_with("name: p\n"),
        "a profile reads in written order, not alphabetical: {text}"
    );
    let back = Profile::from_text(&text).expect("emitted YAML parses back");
...
mod git_backed {
    ...
    fn a_spec_pins_a_tag_from_the_repo() {
        let (repo, first) = init_repo_with(
            "capabilities/greet/capability.yaml",
            &cap_yaml(
                "greet",
                "1.0.0",
                "skill",
        ...
        write(
            &repo.join("capabilities/greet/capability.yaml"),
            &cap_yaml(
                "greet",
                "2.0.0",
                "skill",
